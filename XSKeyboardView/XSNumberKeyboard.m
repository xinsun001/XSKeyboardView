...
#define downH 10*[UIScreen mainScreen].scale
...
@interface XSNumberKeyboard () <UIInputViewAudioFeedback>
{
    NSTimer *deleteTimer;
}
...
@implementation XSNumberKeyboard
...
- (void)layoutSubviews
{
    ...
    CGFloat height = (CGRectGetHeight(self.bounds)-40-keyboardH )/ 4.f - .5f;
    
    CGFloat inputH=downH;
    
    [self viewWithTag:-3].frame=CGRectMake(0, 0, CGRectGetWidth(self.bounds), downH);

    [self viewWithTag:49].frame = CGRectMake(0, inputH+.5, width, height);
    [self viewWithTag:50].frame = CGRectMake(width + .5f, inputH+.5f, width, height);
    [self viewWithTag:51].frame = CGRectMake((width + .5f) * 2, inputH+.5f, width, height);
    [self viewWithTag:52].frame = CGRectMake(0, inputH+height + 1.f, width, height);
    [self viewWithTag:53].frame = CGRectMake(width + .5f, inputH+height + 1.f, width, height);
    [self viewWithTag:54].frame = CGRectMake((width + .5f) * 2, inputH+height + 1.f, width, height);
    [self viewWithTag:55].frame = CGRectMake(0, inputH+.5f + (height + .5f) * 2, width, height);
    [self viewWithTag:56].frame = CGRectMake(width + .5f, inputH+.5f + (height + .5f) * 2, width, height);
    [self viewWithTag:57].frame = CGRectMake((width + .5f) * 2, inputH+.5f + (height + .5f) * 2, width, height);
    ...
    [self viewWithTag:-1].frame = CGRectMake((width + .5f) * 3, inputH+.5f + (height + .5f) * 3, 0, height);

    
    [self viewWithTag:127].frame = CGRectMake((width + .5f) * 3, inputH+.5f, width, height + .5f);
    [self viewWithTag:-2].frame = CGRectMake((width + .5f) * 3, inputH+1.f + height, width, height * 3 + 1.f);
